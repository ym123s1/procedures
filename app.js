...
const PROTOCOLS = [
    ...
    },
    {
        ...
        pdfs: [
            { path: "קבצים/קולונוסקופיה (פיקולקס).PDF", text: "קולונוסקופיה (פיקולקס).PDF" },
            { path: "קבצים/קולונוסקופיה (מרוקן).PDF", text: "קולונוסקופיה (מרוקן).PDF" },
            { path: "קבצים/סיגמואידוסקופיה.PDF", text: "סיגמואידוסקופיה.PDF" }
        ]
    },
    {
    ...
    {
        question: "הנחיות צום והכנה לבדיקות PET CT",
        answer: "(PET CT) - סוג הבדיקה נקבע באישור של היחידה לרפואה גרעינית.   שני סוגים נפוצים:   (PET CT) קרדיאלי - עשר שעות צום מלא, למעט מים ותרופות הכרחיות. אין לתת דקסטרוז דרך הווריד ב24ש' קודם הבדיקה. בנוסף - עירוי.   (PET CT (FDG)) - חמש שעות טרם הבדיקה צום מלא למעט מים.",
        pdfPath: "קבצים/PET CT קרדיאלי.PDF",
        pdfText: "PET CT קרדיאלי.PDF"
    },
    {
        question: "הנחיות צום והכנה למיפוי עצמות ומיפוי ריאות",
        answer: "רפואה גרעינית   מיפוי ריאות + מיפוי עצמות - אין צורך בצום, נדרש עירוי."
    },
    {
    ...
    {
        question: "רשימת ציוד LP",
        answer: "פרוצדורות רפואיות עזרה של אחות  - ציוד ל-LP   אלכוכסידין   מזרק 10 CC   מחט שאיבה LP   מחט שאיבה   מחט הזרקה   לידוקאין   מבחנות סטריליות (פקק אדום/ שחור)"
    }
];
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    PROTOCOLS.forEach((protocol, index) => {
        ...
        // Render PDF attachments if configured
        if (protocol.pdfs || protocol.pdfPath) {
            const pdfContainer = document.createElement('div');
            pdfContainer.className = 'pdf-buttons-container';
            
            const createPdfButton = (path, text) => {
                const btn = document.createElement('a');
                btn.href = path;
                btn.target = '_blank';
                btn.className = 'pdf-download-btn';
                btn.innerHTML = `
                    <svg class="pdf-icon" viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
            ...
            if (protocol.pdfs && Array.isArray(protocol.pdfs)) {
                protocol.pdfs.forEach(pdfInfo => {
                    pdfContainer.appendChild(createPdfButton(pdfInfo.path, pdfInfo.text));
                });
            } else if (protocol.pdfPath) {
                pdfContainer.appendChild(createPdfButton(protocol.pdfPath, protocol.pdfText || 'לצפייה במסמך ה-PDF.PDF'));
            }
            
            inner.appendChild(pdfContainer);
        }
